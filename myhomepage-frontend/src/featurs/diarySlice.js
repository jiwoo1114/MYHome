    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(updateDiaryThunk.fulfilled, (state, action) => {
                ...
                if (index !== -1) state.diaries[index] = action.payload; // 수정된 다이어리 업데이트
            })
            ...
            .addCase(deleteDiaryThunk.fulfilled, (state, action) => {
                state.loading = false;
                state.diaries = state.diaries.filter((d) => d.id !== action.payload); // 삭제된 다이어리 제거
            })
            ...
            .addCase(fetchDiariesThunk.fulfilled, (state, action) => {
                state.loading = false;
                state.diaries = action.payload.posts; // 다이어리 리스트 저장
                state.pagination = action.payload.pagination; // 페이징 정보 저장
            })
